...
class Pdb(object):
    ...
    def ss_for_seq(self, c = 'X'):
        seq_struct = self.seq_from_structure()
        seq_full = self.seq_from_header()
        ss_struct = self.ss_for_struct()
        if len(seq_struct) == len(seq_full):
            ss_full = ss_struct
        else:
            ss_full = 'C'*len(seq_full)
            s = list(ss)
            ids = self.resi_ids()
            missing = self.missing_resi()
            ch_r = self.chain_ranges()
            first = 0
            if int(missing[0][2]) < int(ids[0]):
                first = int(missing[0][2])
            else:
                first = int(ids[0])
            for m in missing:
                for ch in ch_r:
    ...
    def compare_seqs(self):
        seq_struct = list(self.seq_from_structure())
