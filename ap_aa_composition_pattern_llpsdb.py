...
out6.write("# protein_name".rjust(20)+" "+"SCD".rjust(9)+" "+"OCS".rjust(9)+"\n")
    ...
    for protein in db.keys():
        if protein in pnames:
            ...
            if len(seq) > 0:
                ...
                out6.write(str(protein).rjust(20)+" "+str(round(sequence_charge_decoration(seq), 4)).ljust(6, '0').rjust(9)+" "+str(round(overall_charge_symmetry(seq)[1], 4)).ljust(6, '0').rjust(9)+"\n")
    out1.close()
    out2.close()
    out3.close()
    out4.close()
